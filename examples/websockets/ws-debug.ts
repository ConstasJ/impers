...
async function main() {
  ...
  // Set URL - use local mock server
  // Start the mock server first, default port from args or 8765
  const port = process.argv[2] || "8765";
  const url = `http://127.0.0.1:${port}/ws/echo`;
  ...
  // Set write callback
  curl_easy_setopt(handle, CURLOPT_WRITEFUNCTION, "void *", writeCallback);
